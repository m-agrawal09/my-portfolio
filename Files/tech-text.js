/**
 * TechText Animation - Faithful Vanilla Engine of React Bits <TechText />
 * Source: https://reactbits.dev/text-animations/tech-text?text=MEDHAVI+AGRAWAL&letterSpacing=-0.03&fontWeight=700&strokeWidth=1.75&speed=1.1
 */
...
export function initTechText(target, userOptions = {}) {
  const container = typeof target === 'string' ? document.querySelector(target) : target;
  if (!container) return null;

  const defaults = {
    text: 'MEDHAVI AGRAWAL',
    fontFamily: '',
    fontWeight: 700,
    fontSize: 150,
    letterSpacing: -0.03,
  ...
  const getColor = () => {
    if (userOptions.color && userOptions.color !== 'auto') return userOptions.color;
    return isLightMode() ? '#15181D' : '#F2F1EC';
  };
  ...
  let canvas = container.querySelector('canvas.tech-text-canvas');
  if (!canvas) {
  ...
  const family = () => s.fontFamily || getComputedStyle(container).fontFamily || 'Manrope, sans-serif';
  const fontFor = size => `${s.fontWeight} ${size}px ${family()}`;

  const setFont = (targetCtx, size) => {
    targetCtx.font = fontFor(size);
    if ('letterSpacing' in targetCtx) targetCtx.letterSpacing = `${s.letterSpacing * size}px`;
  ...
  const ensureLayout = () => {
    ...
    const x = s.align === 'center'
      ? (width - inkWidth) / 2 + m.actualBoundingBoxLeft
      : m.actualBoundingBoxLeft + 4;
  ...
  const falloff = (targetCtx, cx, cy, radius, strength, softness) => {
  ...
  const blit = (targetCtx, art, dx, dy, originX, originY) => {
    targetCtx.drawImage(
      art.image,
  ...
  const drawSpecks = a => {
    ...
    for (let k = 0; k < s.specks; k++) {
      ...
      if (tone < 0.26 || (large && tone < 0.78)) {
        ctx.strokeStyle = rgba(currentAccent, alpha);
    ...
    for (let j = 0; j < 2; j++) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const size = i === 0 ? 3 : 2;
        ctx.fillStyle = rgba(currentAccent, a * [0.95, 0.55, 0.32, 0.16][i]);
  ...
  const drawFrame = () => {
    ...
    if (moved > 1) {
      ...
      ctx.setLineDash([3, 4]);
      ctx.lineWidth = 1;
      ctx.strokeStyle = rgba(currentAccent, 0.45 * a);
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.beginPath();
      ctx.rect(Math.round(hx) - 2, Math.round(hy) - 2, 4, 4);
      ctx.fillStyle = rgba(currentAccent, 0.7 * a);
      ctx.fill();
    }
    ...
    ctx.lineWidth = 1;
    ctx.strokeStyle = rgba(currentAccent, 0.5 * a);
    ctx.stroke();
    ...
    ctx.fillStyle = rgba(currentAccent, 0.95 * a);
    ctx.fill();
    ...
    ctx.fillStyle = rgba(currentAccent, 0.62 * a);
    const label =
      moved > 1
  ...
  // Watch for light/dark theme attribute changes on <html>
...
if (typeof window !== 'undefined') {
  window.initTechText = initTechText;
}
